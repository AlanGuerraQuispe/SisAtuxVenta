...
public abstract class JAbstractController<M extends JAbstractModelBD> implements JController{
    ...
    {
        String sqlSelect = "SELECT "+nombreColumna+" FROM "+nombreTabla+" ORDER BY "+nombreColumna+" DESC LIMIT 1";
        String codigo = null;
        try{            
            rs = BaseConexion.getResultSet(sqlSelect);
            while(rs.next())
            {
    ...
    public int eliminacionFoto(String nombreTabla, String coCompania, String coLocal, String nuSecUsuario){
        sql = "delete from " + nombreTabla + " where co_compania ='" + coCompania + "' and co_local = '" + coLocal + "' and Nu_Sec_Usuario = '" + nuSecUsuario + "'";
        int opcion = 0;
        try{           
            opcion = BaseConexion.getStatement().executeUpdate(sql);
            BaseConexion.cerrarEnlacesConexion(BaseConexion.SOLO_STATEMENT);
    ...
    public int eliminacionTemporal(String nombreTabla,String nomColumnaCambiar,String[] nomColumnaId,Object[] id,Object nuevoValor)
    {
        ...
        */
        ...
        String condiciones = unirColumnasValores(nomColumnaId,id);
        //TODO aguerra ver uso en mysql
        //String sql = "UPDATE "+nombreTabla+" SET "+nomColumnaCambiar+" = "+(nv==null?nuevoValor:nv)+" where "+nomColumnaId+"="+(vl==null?id:vl)+"";                
        String sqlupdate = "UPDATE "+nombreTabla+" SET "+nomColumnaCambiar+" = "+(nv==null?nuevoValor:nv)+(condiciones!=null?Ex.WHERE+condiciones:"");                        
                
        try{            
            resultado = BaseConexion.getStatement().executeUpdate(sqlupdate); 
        }catch(SQLException ex){
    ...
    public boolean esCadena(Object valor)
    {
    ...
    public ResultSet getRegistros(String nombreTabla, String[] campos, String[] columnaId, Object[] id,String[] columOrder) {
        ...
        sql = "select "+(campos!=null?(campos.length==0?"* ":generarArrayAString(campos)):"* ")+" from "+
               nombreTabla+(condiciones!=null?Ex.WHERE+condiciones:"")+""+Ex.ORDER_BY +""+(columOrder!=null?(columOrder.length==0?"1":generarArrayAString(columOrder)):"1");
               //+Ex.LIMIT+this.inicioPag+","+this.finalPag;  TODO aguerra solo en mysql        
    ...
    public ResultSet getRegistrosMn(String nombreTabla, String[] campos, String[] columnaId, Object[] id) throws SQLException {
    ...
    public ResultSet getRegistros(String nombreTabla, String[] columnaId, Object[] id) {
        return getRegistros(nombreTabla,null, columnaId,id,null);
    }
    ...
    public ResultSet getRegistros(String nombreTabla, String[] campos) {
        return getRegistros(nombreTabla,campos, null, null,null);
    }
    ...
    public ResultSet getRegistrosRs(String nombreTabla) {
        return getRegistros(nombreTabla,null, null, null,null);
    }
     ...
     * @param valores campos, valores a grabar
     * @return 
     */
     ...
     * @return 
     */
    public int agregarRegistroPs(String nombreTabla, String[] campos, Object[] valores) throws SQLException {
        
        sql = "insert into "+nombreTabla+"("+generarArrayAString(campos)+") values("+formatearValores(valores.length)+")";
    ...
    public ResultSet getUltimoRegistro(String table,String campos) throws SQLException
    {
        sql = "select * from "+table+" order by "+campos+" desc limit 0,1";
        rs = BaseConexion.getResultSet(sql);
        return rs;
    }
        ...
        try{
            for(int i=0;i<valores.length;i++) 
            {
                ...
                }else if(valores[i] instanceof DatoArchivo)
                {
                   ...
                   //System.out.println(i+1+" "+da.getLongitud());
                }
                 else if(getString(valores[i]) != null)
                {                    
                       ps.setString(i+1,valores[i].toString()); 
                       //System.out.println(i+1+" "+valores[i].toString());
                }
                else if(getTimestamp(valores[i]) != null)
                {
                    ps.setTimestamp(i+1 ,getTimestamp(valores[i]));
                }
                else if(getDate(valores[i]) != null)
                {
                    ps.setDate(i + 1, getDate(valores[i]));
                    //System.out.println(i+1+" "+getDate(valores[i]));
                }
                else {
    ...
    private java.sql.Date getDate(Object valor)
    {
        java.sql.Date vl = null;
        if(valor instanceof java.util.Date) 
        {
            vl = new java.sql.Date(((java.util.Date)valor).getTime());
        }
        return vl;
    }

    private java.sql.Timestamp getTimestamp(Object valor)
    {
        java.sql.Timestamp vl = null;
        if(valor instanceof java.sql.Timestamp)
        {
            vl = new java.sql.Timestamp(((java.sql.Timestamp)valor).getTime());
        }
        return vl;
    }
    ...
    // TODO aguerra ver uso del where en mysql y oracle
    ...
    private ResultSet getRegistroPk(String nombreTabla,String[] campos,String columnaid,Integer id) throws SQLException
    {
    ...
    public ResultSet getRegistrosSinFiltro(StringBuffer sql) throws SQLException
    {   
//        System.out.println(sql.toString());
        rs = BaseConexion.getResultSet(sql.toString());
        return rs;
    }
        
    public ResultSet selectPorPk(String nombreTabla, String[] campos, String[] columnaId, Object[] id) throws SQLException
    {
        return this.getRegistros(nombreTabla, campos, columnaId, id,null);
    }
    ...
    public abstract boolean guardarRegistro(M mdl) throws SQLException;
    public abstract int actualizarRegistro(M mdl)  throws SQLException;
